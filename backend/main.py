...
FW_MODEL = os.getenv("WHISPER_MODEL", "large-v3")  # ← large-v3 に
FW_DEVICE = os.getenv("WHISPER_DEVICE", "cuda")    # CPUなら "cpu"
FW_COMPUTE_TYPE = os.getenv(
    "WHISPER_COMPUTE_TYPE",
    "float16" if FW_DEVICE == "cuda" else "int8"   # CPUは int8 が無難
)

whisper_model = WhisperModel(FW_MODEL, device=FW_DEVICE, compute_type=FW_COMPUTE_TYPE)
...
SYSTEM_PROMPT = (
    ...
    "・出力は簡潔に短く。\n"
    "・口調は丁寧で公平、やや厳しめ。・質問は一度に1問だけ。・解説は控えめで総評は終了時。\n"
...
def stt_whisper(wav_path: Path) -> str:
    """faster-whisper で文字起こし（日本語想定）"""
    segments, info = whisper_model.transcribe(
        str(wav_path),
        language="ja",
        beam_size=5,
        vad_filter=False,  # 無音区間が多い音声で安定しやすい（不要なら False）
    )
